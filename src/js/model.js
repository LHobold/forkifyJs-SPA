...
export function updateServings(newServings) {
  const servingsRatio = newServings / state.recipe.servings;
  state.recipe.ingredients.forEach(ing => {
    ing.quantity = ing.quantity * servingsRatio;
  });
  state.recipe.servings = newServings;
}

export function addBookmark(recipe) {
  // if (recipe.id === state.recipe.id && state.recipe.bookmarked) return;
